# services_with_logging.py
import datetime
import json
import database
from models import Task, Event, Models # Assuming these are Pydantic or similar, not directly DB models
...
class TaskService:
    ...
    def create_task(user_id, data):
        try:
            # Assuming Models.validate_task and Task.create handle data validation and object creation
            # These should raise ValueError or similar for bad data, caught by the caller or here.
            validated_data = Models.validate_task(data)
            task_obj_data = Task.create( # This should return a dict or an object with attributes
                user_id=user_id,
                title=validated_data['title'],
                description=validated_data.get('description'),
                due_datetime=validated_data.get('due_datetime'), # Should be ISO format string
                priority=validated_data.get('priority', 'medium')
            )
            # If Task.create returns an object, convert to dict for DB
            ...
            cursor.execute(
                ...
                (task_data_for_db['task_id'], user_id, task_data_for_db['title'],
                 ...
                 task_data_for_db['priority'], task_data_for_db.get('status', 'pending')) # Ensure status
            )
            db.commit()
            ...
            # db.rollback() # Handled by database connection context or if not using autocommit
            return None # Or raise ServiceError(f"Failed to create task: {str(e)}")
    ...
    def complete_task(task_id, user_id):
        try:
            ...
            cursor.execute( # Changed from db.execute to cursor.execute
                "UPDATE tasks SET status = 'completed', status_change_timestamp = ? WHERE task_id = ?",
                (completed_at, task_id)
            )
            db.commit()

            try: # Separate try-except for non-critical AdaptiveLearner
...
class EventService:
    ...
    def create_event(user_id, data):
        try:
            ...
            event_obj_data = Event.create(user_id=user_id, **validated_data) # Pass validated data directly
            if not isinstance(event_obj_data, dict):
                ...
                event_data_for_db = event_obj_data
            ...
            db.commit()
            logger.info(f"Event created: {event_data_for_db['event_id']} for user {user_id}")
            return event_data_for_db
        except Exception as e:
            logger.error(f"Database error in EventService.create_event for user {user_id}: {str(e)}", exc_info=True)
            return None
    ...
    def get_all_events(user_id):
        try:
            ...
            logger.error(f"Database error in EventService.get_all_events for user {user_id}: {str(e)}", exc_info=True)
            return []

class LearningService:
    def create_learning_session(self, user_id, topic, resources, start_datetime_str):
        try:
            event_service = EventService() # Assuming EventService handles its own errors
            ...
            if not created_event:
                # Error already logged by EventService, or raise specific error here
                raise ServiceError(f"Failed to create event for learning session for topic '{topic}'.")
            ...
            cursor.execute( # Changed from db.execute
                "INSERT INTO notes (note_id, user_id, title, content, tags) VALUES (?, ?, ?, ?, ?)",
                (note_id, user_id, note['title'], note['content'], note['tags'])
            )
            db.commit()

            full_note_details = {"note_id": note_id, **note}
            ...
            try: # Separate try-except for non-critical AdaptiveLearner
                ...
                logger.error(f"AdaptiveLearner error in create_learning_session for user {user_id}, topic {topic}: {str(learner_e)}", exc_info=True)

            return created_event, full_note_details
        except ValueError as ve: # Specifically for fromisoformat or strptime if used
            logger.error(f"Date/time format error in LearningService.create_learning_session for user {user_id}, topic {topic}: {str(ve)}", exc_info=True)
            raise ServiceError(f"Invalid start date/time format provided for learning session.")
        except Exception as e:
            logger.error(f"Error in LearningService.create_learning_session for user {user_id}, topic {topic}: {str(e)}", exc_info=True)
            # db.rollback() # If any db operations were performed directly in this block before commit
            raise ServiceError(f"Could not create learning session: {str(e)}")
    ...
    def generate_personalized_content(self, user_id, topic):
        try:
            ...
            content = ""
            if style == "visual": content = f"Visual learning resources for {topic}:\n- Diagrams\n- Infographics\n- Video tutorials"
            elif style == "auditory": content = f"Auditory learning resources for {topic}:\n- Podcasts\n- Lectures\n- Audio discussions"
            else: content = f"General resources for {topic}:\n- Practice exercises\n- Projects\n- In-depth readings"
            content += "\n\nStarting with general materials..."
            return content
        except Exception as e:
            logger.error(f"Error in LearningService.generate_personalized_content for user {user_id}, topic {topic}: {str(e)}", exc_info=True)
            return "Sorry, I encountered an error generating personalized content. Please try again."
...
class ReportService:
    ...
    def generate_daily_summary(user_id):
        try:
            ...
            cursor = db.cursor()
            cursor.execute(
                "SELECT title, start_datetime FROM events WHERE user_id = ? AND date(start_datetime) = ? AND (is_archived = 0 OR is_archived IS NULL) ORDER BY start_datetime",
                (user_id, today)
            )
            events = cursor.fetchall()
            cursor.execute(
                "SELECT title, priority FROM tasks WHERE user_id = ? AND status != 'completed' AND (is_archived = 0 OR is_archived IS NULL) ORDER BY due_datetime",
                (user_id,)
            )
            tasks = cursor.fetchall()
            ...
            logger.error(f"Database error in ReportService.generate_daily_summary for user {user_id}: {str(e)}", exc_info=True)
