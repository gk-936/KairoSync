...
def get_db_connection():
    ...
    if _db_connection is None:
        try:
            ...
            _db_connection.execute("PRAGMA foreign_keys = ON;") # Enable foreign key enforcement
    ...
    try:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS tasks (
                ...
                scheduled_end TEXT,
                FOREIGN KEY(depends_on_task_id) REFERENCES tasks(task_id) ON DELETE SET NULL ON UPDATE CASCADE -- Added
            );''')
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_tasks_depends_on ON tasks(depends_on_task_id);") # Index for foreign key
...
if __name__ == '__main__':
    ...
    try:
        ...
        fk_check = test_cursor.execute("PRAGMA foreign_keys;").fetchone()
        logger.info(f"Foreign key enforcement check (1 is ON): {fk_check[0] if fk_check else 'Could not check'}")
